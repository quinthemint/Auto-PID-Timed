...
public final class Vector2 implements Interpolable<Vector2>, Serializable {
    ...
    public static Rotation2 getAngleBetween(Vector2 a, Vector2 b) {
        float cos = a.dot(b) / (a.length * b.length);
        if (Float.isNaN(cos)) {
            return Rotation2.ZERO;
        }

        return Rotation2.fromRadians((float) Math.acos(MathUtils.clamp(cos, (float) -1.0, (float) 1.0)));
    }
    ...
    /**
     * Calculates the cross product of this vector and another vector in 3d space
    ...
    public boolean equals(Vector2 other, float allowableError) {
    ...
    @Override
    public Vector2 interpolate(Vector2 other, float t) {
        if (t <= 0.0) {
            return this;
        } else if (t >= 1.0) {
            return other;
    ...
    public Vector2 extrapolate(Vector2 other, float t) {
