...
/**
 * This represents a rotation in 3d space through a rotation matrix and its
...
public class Rotation3 {
     ...
     */
    public Rotation3(float[][] rotationMatrix) {
     ...
     * 
     * @param eulerAngles A float array of euler angles
     */
    public Rotation3(float[] eulerAngles) {
    ...
    /**
     * Calculates the transpose of the rotation matrix, which is equal to it's
     * inverse
     * 
     ...
     */
    private static float[][] getRotationMatrix(float[] eulerAngles) {
        float[][] result_x = new float[][] { { 1, 0, 0 },
                { 0, (float) Math.cos(eulerAngles[0]), (float) -Math.sin(eulerAngles[0]) },
                { 0, (float) Math.sin(eulerAngles[0]), (float) Math.cos(eulerAngles[0]) } };

        float[][] result_y = new float[][] { { (float) Math.cos(eulerAngles[1]), 0, (float) Math.sin(eulerAngles[1]) },
                { 0, 1, 0 }, { (float) -Math.sin(eulerAngles[1]), 0, (float) Math.cos(eulerAngles[1]) } };

        float[][] result_z = new float[][] { { (float) Math.cos(eulerAngles[2]), (float) -Math.sin(eulerAngles[2]), 0 },
                { (float) Math.sin(eulerAngles[2]), (float) Math.cos(eulerAngles[2]), 0 }, { 0, 0, 1 } };
     ...
     */
    private static float[] getEulerAngles(float[][] rotationMatrix) {
        float sy = (float) Math.sqrt(Math.pow(rotationMatrix[0][0], 2) + Math.pow(rotationMatrix[1][0], 2));
        boolean singular = sy < 1e-6;
        float x, y, z;
        if (singular) {
            x = (float) Math.atan2(-rotationMatrix[1][2], rotationMatrix[1][1]);
            y = (float) Math.atan2(-rotationMatrix[2][0], sy);
            z = 0;
        } else {
            x = (float) Math.atan2(rotationMatrix[2][1], rotationMatrix[2][2]);
            y = (float) Math.atan2(-rotationMatrix[2][0], sy);
            z = (float) Math.atan2(rotationMatrix[1][0], rotationMatrix[0][0]);
        }
    ...
    public String toString() {
        ...
        return "Rotation " + "- X: " + fmt.format(Math.toDegrees(eulerAngles[0])) + '\u00b0' + ", Y: "
                + fmt.format(Math.toDegrees(eulerAngles[1])) + '\u00b0' + ", Z: "
