...
public class SwerveDriveKinematics {
     ...
     * of wheel locations as Translation2ds. The order in which you pass in the
     * wheel locations is the same order that you will recieve the module states
     * when performing inverse kinematics. It is also expected that you pass in the
     * module states in the same order when calling the forward kinematics methods.
     *
     ...
     * if you wish to change the center of rotation for evasive manuevers, vision
     ...
     *                               the center of rotation at one corner of the
     *                               robot and provide a chassis speed that only has
     *                               a dtheta component, the robot will rotate
     *                               around that corner.
     ...
     *         the {@link #normalizeWheelSpeeds(SwerveModuleState[], float)
    ...
    public SwerveModuleState[] toSwerveModuleStates(ChassisSpeeds chassisSpeeds, Translation2d centerOfRotationMeters) {
        ...
        for (int i = 0; i < m_numModules; i++) {
            float x = (float) moduleStatesMatrix.get(i * 2, 0);
            float y = (float) moduleStatesMatrix.get(i * 2 + 1, 0);

            float speed = (float) Math.hypot(x, y);
     ...
     *                    the swerve module states should be same as passed into the
    ...
    public ChassisSpeeds toChassisSpeeds(SwerveModuleState... wheelStates) {
        ...
        return new ChassisSpeeds((float) chassisSpeedsVector.get(0, 0), (float) chassisSpeedsVector.get(1, 0),
                (float) chassisSpeedsVector.get(2, 0));

    }

    /**
     * Normalizes the wheel speeds using some max attainable speed. Sometimes, after
     * inverse kinematics, the requested speed from a/several modules may be above
     * the max attainable speed for the driving motor on that module. To fix this
     * issue, one can "normalize" all the wheel speeds to make sure that all
     ...
     *                                          can reach.
     */
    public static void normalizeWheelSpeeds(SwerveModuleState[] moduleStates, float attainableMaxSpeedMetersPerSecond) {
        float realMaxSpeed = (float) Collections.max(Arrays.asList(moduleStates)).speedMetersPerSecond;
