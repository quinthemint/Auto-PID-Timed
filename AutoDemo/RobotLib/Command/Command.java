 ...
 * Commands are run by the {@link CommandScheduler}, and can be composed into
...
public interface Command {
    ...
    /**
     * Specifies the set of subsystems used by this command. Two commands cannot use
     * the same subsystem at the same time. If the command is scheduled as
     * interruptible and another command is scheduled that shares a requirement, the
     * command will be interrupted. Else, the command will not be scheduled. If no
     * subsystems are required, return an empty set.
     *
     * <p>
     * Note: it is recommended that user implementations contain the requirements as
     * a field, and return that field here, rather than allocating a new set every
     * time this is called.
     *
    ...
    /**
     * Decorates this command with a timeout. If the specified timeout is exceeded
     * before the command finishes normally, the command will be interrupted and
     * un-scheduled. Note that the timeout only applies to the command returned by
     * this method; the calling command is not itself changed.
     *
     * <p>
     * Note: This decorator works by composing this command within a CommandGroup.
     * The command cannot be used independently after being decorated, or be
     * re-decorated with a different decorator, unless it is manually cleared from
     * the list of grouped commands with
     * {@link CommandGroupBase#clearGroupedCommand(Command)}. The decorated command
     * can, however, be further decorated without issue.
     *
     * @param seconds the timeout duration
     * @return the command with the timeout added
     */
    default ParallelRaceGroup withTimeout(float seconds) {
        return new ParallelRaceGroup(this, new WaitCommand(seconds));
    }

    /**
     * Decorates this command with an interrupt condition. If the specified
     * condition becomes true before the command finishes normally, the command will
     * be interrupted and un-scheduled. Note that this only applies to the command
     ...
     * Often more convenient/less-verbose than constructing a new
     * {@link SequentialCommandGroup} explicitly.
     *
     * <p>
     * Note: This decorator works by composing this command within a CommandGroup.
     * The command cannot be used independently after being decorated, or be
     ...
     * convenient/less-verbose than constructing a new {@link ParallelDeadlineGroup}
     * explicitly.
     *
     * <p>
     * Note: This decorator works by composing this command within a CommandGroup.
     ...
     * {@link ProxyScheduleCommand}. This is useful for "forking off" from command
     * groups when the user does not wish to extend the command's requirements to
     * the entire command group.
     *
     * @return the decorated command
     */
    ...
    /**
     * Cancels this command. Will call the command's interrupted() method. Commands
     * will be canceled even if they are not marked as interruptible.
     */
    default void cancel() {
     ...
     * detect whether the command is being run by a CommandGroup, only whether it is
     * directly being run by the scheduler.
     *
    ...
    /**
     * Whether the command requires a given subsystem. Named "hasRequirement" rather
     * than "requires" to avoid confusion with
    ...
    /**
     * Whether the given command should run when the robot is disabled. Override to
     * return true if the command should run when disabled.
     *
