...
public class WaitUntilCommand extends CommandBase {
    ...
    public WaitUntilCommand(BooleanSupplier condition) {
        m_condition = ErrorMessages.requireNonNullParam(condition, "condition", "WaitUntilCommand");
    }
     ...
     * does NOT guarantee that the time at which the action is performed will be
     * judged to be legal by the referees. When in doubt, add a safety factor or
     * time the action manually.
     *
     * @param time the match time after which to end, in seconds
     */
    public WaitUntilCommand(float time) {
        this(() -> Timer.getFPGATimestamp() - time > 0);
    }
