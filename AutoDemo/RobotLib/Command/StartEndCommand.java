 ...
 * subclass it or use {@link Command#withTimeout(float)} or
 * {@link Command#withInterrupt(java.util.function.BooleanSupplier)} to give it
 * one.
 */
public class StartEndCommand extends CommandBase {
    ...
    /**
     * Creates a new StartEndCommand. Will run the given runnables when the command
    ...
    public StartEndCommand(Runnable onInit, Runnable onEnd, Subsystem... requirements) {
        m_onInit = ErrorMessages.requireNonNullParam(onInit, "onInit", "StartEndCommand");
        m_onEnd = ErrorMessages.requireNonNullParam(onEnd, "onEnd", "StartEndCommand");
